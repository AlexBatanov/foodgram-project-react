...
class SubscriptionsView(viewsets.ViewSet):
    ...
    def get_subscribers(self, request):
        ...
        authors = User.objects.filter(
            subscribers__in=Subscription.objects.filter(user=user)
        )
    ...
    def subscribe(self, request, pk):
        ...
        if request.method == 'POST':
            ...
            if author == request.user:
                return Response(data='Подписываться на себя нельзя',
                                status=status.HTTP_400_BAD_REQUEST)

            if subscription:
                return Response(data='Уже подписан',
                                status=status.HTTP_400_BAD_REQUEST)
        ...
        return Response(data='Нет в подписках',
                        status=status.HTTP_400_BAD_REQUEST)
